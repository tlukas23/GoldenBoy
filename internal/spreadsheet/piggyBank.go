...
func AnalyzeTheSheet(spreadsheet [][]string, teamsStats map[string]schemas.KPTeamStats) []schemas.GoldenCopyRow {
	...
	for _, row := range spreadsheet {
		...
		htLog5PredPts, atLog5PredPts := oddsCalc.CalculateLog5KpPointMargin(htStats, atStats)
		...
		htRow := schemas.GoldenCopyRow{
			...
			HardRockWinPercentage: strconv.FormatFloat(htHRWp, 'f', -1, 64),
			...
			KpPointMargin:         strconv.FormatFloat(htKpPointDiff, 'f', -1, 64),
		...
		atRow := schemas.GoldenCopyRow{
			...
			HardRockWinPercentage: strconv.FormatFloat(atHRWp, 'f', -1, 64),
			...
			KpPointMargin:         strconv.FormatFloat(atKpPointDiff, 'f', -1, 64),
